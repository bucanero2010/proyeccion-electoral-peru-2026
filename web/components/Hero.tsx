  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5">
        ...
        <p className="mt-1 text-xs text-zinc-500">Sobre votos válidos proyectados</p>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-zinc-100 pt-4 text-sm">
          <div>
            <dt className="text-xs text-zinc-500">Votos FP proyectados</dt>
            <dd className="font-medium text-[var(--fp)]">{fmtInt(fp.votos_proyectados)}</dd>
            <dd className="text-xs text-zinc-500">{fmtPct(fp.pct_proyectado, 2)}</dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-500">Votos JP proyectados</dt>
            <dd className="font-medium text-[var(--jp)]">{fmtInt(jp.votos_proyectados)}</dd>
            <dd className="text-xs text-zinc-500">{fmtPct(jp.pct_proyectado, 2)}</dd>
          </div>
        </dl>
      </div>
    </section>
    ...
    <div>
      <div className="mb-1.5 flex items-baseline justify-between gap-3">
        <div className="flex items-baseline gap-2">
          <span
            className={`inline-flex items-center justify-center rounded-md px-1.5 py-0.5 text-xs font-semibold text-white ${color}`}
            aria-hidden
          >
            {shortLabel}
          </span>
        ...
        <span
          className={`text-2xl font-semibold tracking-tight ${
            highlight ? "text-zinc-900" : "text-zinc-500"
          }`}
        >
          {fmtPct(value, 2)}
        </span>
